...
export default function RoleLoginPage({
  ...
  const handleRequestOtp = async () => {
    try {
      ...
      setLoading(true);
      const payload = await requestParentOtp(slug!, parentPhone.trim());
      setChallengeId(payload.challengeId);
      const devCode = payload.devOtpCode
        ? ` (DEV OTP: ${payload.devOtpCode})`
        : "";
      setInfo(`Verification code sent${devCode}`);
    } catch (e: unknown) {
      setError(getLocalizedError(e, "otpRequestFailed"));
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (type === "SUPER_ADMIN") {
        ...
        session = await loginParent(
          ...
          parentPhone.trim(),
          useOtp
            ? { otpCode: otpCode.trim(), challengeId: challengeId.trim() }
            : { password },
        );
      }
            ...
            {type === "PARENT" && (
              <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <MessageSquare className="w-3.5 h-3.5 inline mr-1" /> OTP
                  </button>
                  <button
                    type="button"
                ...
                {useOtp ? (
                  <>
                    <button
                      ...
                      disabled={loading}
                      className="w-full bg-gray-900 text-white font-semibold py-2.5 rounded-xl hover:bg-black transition-colors disabled:opacity-70 text-sm"
                    >
                      Request OTP
                    </button>
                    <div>

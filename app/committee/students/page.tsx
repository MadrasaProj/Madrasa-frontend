  ...
  const statCards = [
    {
      ...
      sub:
        lang === "ml"
          ? `${d.overview.totalClasses} ക്ലാസ്സുകൾ`
          : `${d.overview.totalClasses} classes`,
      icon: Users,
    ...
    <DashboardLayout>
      ...
      >
        <div className="bg-linear-to-r from-violet-700 to-blue-600 rounded-3xl p-5 lg:p-6 text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-xl font-bold">
                {lang === "ml"
                  ? "വിദ്യാർത്ഥി & അക്കാദമിക് അവലോകനം"
          ...
          <div className="grid grid-cols-3 gap-3 mt-4">
            {[
              {
                label: lang === "ml" ? "വിദ്യാർ." : "Students",
                value: d.overview.totalStudents,
              },
              {
                label: lang === "ml" ? "ക്ലാസ്" : "Classes",
                value: d.overview.totalClasses,
              },
              {
                label: lang === "ml" ? "ടീച്ചർ" : "Teachers",
                value: d.overview.totalTeachers,
              },
            ].map((s) => (
              <div
                key={s.label}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-gray-400 uppercase tracking-wide">
                ...
                <th className="text-center pb-2 font-semibold">
                  {lang === "ml" ? "വിദ്യാർ." : "Students"}
                </th>
                <th className="text-left pb-2 font-semibold pl-2">
                  {lang === "ml" ? "ഹാജർ%" : "Attend%"}
                </th>
                <th className="text-left pb-2 font-semibold pl-2">
                  {lang === "ml" ? "സ്കോർ%" : "Score%"}
            ...
            <tbody>
              {(d.academic.classStats || []).map((cls: any, i: number) => (
                <motion.tr
                  key={cls.className}
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1">
                        <Bar
                          ...
                          color={barColor(cls.attendancePct ?? 0)}
                          height="h-2"
                        />
                      </div>
                      <span
                        className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${scoreColor(cls.attendancePct ?? 0)}`}
                      >
                        {cls.attendancePct ?? 0}%
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1">
                        <Bar
                          ...
                          color={barColor(cls.avgScore ?? 0)}
                          height="h-2"
                        />
                      </div>
                      <span
                        className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${scoreColor(cls.avgScore ?? 0)}`}
                      >
                        {cls.avgScore ?? 0}%
                      </span>
                    </div>
                  ...
                  <td className="py-2.5 pl-2 min-w-22.5">
                    <div className="flex items-center gap-2">
                      <div className="flex-1">
                        <Bar
                          value={cls.hwCompletion ?? 0}
                          color={barColor(cls.hwCompletion ?? 0)}
                          height="h-2"
                        />
                      </div>
                      <span
                        className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${scoreColor(cls.hwCompletion ?? 0)}`}
                      >
                        {cls.hwCompletion ?? 0}%
                      </span>
                    </div>
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <p className="font-bold text-gray-800">
            {lang === "ml" ? "മികച്ച വിദ്യാർത്ഥികൾ" : "Top Achievers"}
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {(d.academic.topStudents || []).map((s: any, i: number) => (
            <motion.div
              key={s.name}
      ...
      >
        ...
        <div className="space-y-3">
          {(d.ibadah.ibadahChampions || []).map((c: any, i: number) => (
            <motion.div
              key={c.name}
              initial={{ opacity: 0, x: -8 }}
              animate={{ opacity: 1, x: 0 }}

...
function Bar({
  ...
  return (
    <div
      className={`w-full ${height} bg-gray-100 rounded-full overflow-hidden`}
    >
      <div
...
const categoryColorMap: Record<
  string,
  { bar: string; bg: string; text: string; border: string; ring: string }
> = {
  salaries: {
    bar: "bg-violet-500",
    bg: "bg-violet-50",
...
export default function CommitteeExpensesPage() {
  ...
  const filteredExpenses =
    activeFilter === "all"
      ? allExpenses
      : allExpenses.filter((e) => e.categoryId === activeFilter);
  ...
  function handleSave() {
    ...
    const newItem: ExpenseItem = {
      ...
      date: form.date,
      reference: form.paidTo,
      approvedBy: "",
      status: form.status === "paid" ? "approved" : "pending",
    };
    ...
    <DashboardLayout>
                ...
                <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4 pb-6">
                        ...
                        return (
                          <button
                            ...
                            onClick={() =>
                              setForm((f) => ({
                                ...f,
                                category: cat.id as ExpenseCategoryId,
                              }))
                            }
                          ...
                          >
                            <div
                              className={`w-6 h-6 ${c.bg} rounded-lg flex items-center justify-center`}
                            >
                              <Icon className={`w-3 h-3 ${c.text}`} />
                            </div>
                            <span
                              className={`text-xs font-bold ${form.category === cat.id ? c.text : "text-gray-600"}`}
                            >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder={
                        lang === "ml"
                          ? "ഉദാ: പുതിയ ബെഞ്ച് വാങ്ങൽ"
                          : "e.g. New Benches Purchase"
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder={
                        lang === "ml"
                          ? "ഉദാ: National Stationery"
                          : "e.g. National Stationery"
                      }
                      className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm font-medium text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100"
                    />
                  </div>
        ...
        <div className="bg-linear-to-r from-orange-600 to-amber-500 rounded-3xl p-5 lg:p-6 text-white">
          ...
          <div className="bg-white/15 rounded-2xl p-4">
            ...
            <div className="flex justify-between text-xs text-orange-200">
              <span>
                {lang === "ml" ? "ചെലവ്" : "Spent"}: {fmt(totalSpent)}
              </span>
              <span>
                {lang === "ml" ? "ബജറ്റ്" : "Budget"}: {fmt(exp.annualBudget)}
              </span>
            </div>
          </div>
        </div>
          ...
          {exp.categories.map((cat, i) => {
            ...
            return (
              <motion.button
                ...
                onClick={() =>
                  setActiveFilter(
                    activeFilter === cat.id ? "all" : (cat.id as FilterId),
                  )
                }
              ...
              >
                ...
                <Bar
                  value={used}
                  color={over ? "bg-red-500" : c.bar}
                  height="h-2"
                />
                <div className="flex justify-between mt-1.5 text-[10px] text-gray-500 font-medium">
                  <span>
                    {fmt(cat.spent)} {lang === "ml" ? "ചെലവ്" : "spent"}
                  </span>
                  <span>
                    {fmt(cat.budget)} {lang === "ml" ? "ബജറ്റ്" : "budget"}
                  </span>
                </div>
              </motion.button>
            );
      ...
      >
        ...
        <div className="flex items-end gap-2 h-32">
          {exp.monthlyTrend.map((m, i) => {
            const barH =
              maxMonth === 0 ? 0 : Math.round((m.amount / maxMonth) * 100);
            return (
              <motion.div
                key={m.month}
                initial={{ scaleY: 0 }}
      ...
      >
        ...
        <div className="flex gap-1.5 flex-wrap mb-4">
          {(["all", ...exp.categories.map((c) => c.id)] as FilterId[]).map(
            (fid) => {
                  ...
                  className={`text-[11px] font-bold px-2.5 py-1 rounded-full border transition-all ${
                    activeFilter === fid
                      ? fid === "all"
                        ...
                        : `${c?.bar.replace("bg-", "bg-")} text-white border-transparent`
                      : "bg-gray-100 text-gray-600 border-transparent hover:bg-gray-200"
                  }`}
                  style={
                    activeFilter === fid && fid !== "all"
        ...
        <div className="space-y-2">
          {displayedExpenses.map((item, i) => {
            const c =
              categoryColorMap[item.categoryId] ?? categoryColorMap.stationery;
            const Icon = categoryIconMap[item.categoryId] ?? BookOpen;
            const cat = exp.categories.find((cc) => cc.id === item.categoryId);
            return (
              <motion.div
                key={item.id}
                initial={{ opacity: 0, x: -8 }}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 flex-wrap mt-0.5">
                    <span
                      className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${c.bg} ${c.text}`}
                    >
                      {lang === "ml" ? cat?.label_ml : cat?.label}
                    </span>
                    {item.approvedBy && (
                      <span className="text-[10px] text-gray-400">
                        {lang === "ml" ? `അനുമোദനം: ` : `Approved by: `}
                        {item.approvedBy}
                      </span>
                    )}
                    <span className="text-[10px] text-gray-400">
                ...
                <div className="shrink-0 flex flex-col items-end gap-1">
                  ...
                  {item.status === "approved" ? (
                    ...
                    <span className="flex items-center gap-1 text-[10px] font-bold text-red-700 bg-red-100 px-1.5 py-0.5 rounded-full">
                      <AlertTriangle className="w-2.5 h-2.5" />
                      {lang === "ml" ? "നിരസിച്ചു" : "Rejected"}
                    </span>
                  )}
                </div>
              </motion.div>
          ...
          >
            {showAll ? (
              <>
                ...
                {lang === "ml" ? "ചുരുക്കുക" : "Show Less"}
              </>
            ) : (
              <>
                <ChevronDown className="w-4 h-4" />
                {lang === "ml"
                  ? `${filteredExpenses.length - 8} ഇനം കൂടി`
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          {[
            {
              label: lang === "ml" ? "ശമ്പള ബജറ്റ്" : "Salary Budget",
              value: fmt(
                exp.categories.find((c) => c.id === "salaries")?.budget ?? 0,
              ),
              color: "text-violet-700",
            },
            {
              label: lang === "ml" ? "ഇതുവരെ നൽകി" : "Paid So Far",
              value: fmt(
                exp.categories.find((c) => c.id === "salaries")?.spent ?? 0,
              ),
              color: "text-emerald-700",
            },
            {
              label: lang === "ml" ? "ബാക്കി (ഇനിയും)" : "Remaining",
              value: fmt(
                (exp.categories.find((c) => c.id === "salaries")?.budget ?? 0) -
                  (exp.categories.find((c) => c.id === "salaries")?.spent ?? 0),
              ),
            ...
            {
              label: lang === "ml" ? "ഒ. ഉസ്താദ്" : "Total Staff",
              value: `${d.attendance.staff.totalStaff}`,
              color: "text-blue-700",
            },
          ].map((s) => (
        ...
        <div className="space-y-2">
          {(d.attendance.staff.staffList || []).map((staff, i) => {
            const perStaff = Math.round(
              (exp.categories.find((c) => c.id === "salaries")?.spent ?? 0) /
                ((d.attendance.staff.totalStaff || 1) * 5),
            );
            return (
              <motion.div
                key={staff.id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-400 truncate">
                    {lang === "ml" ? staff.role_ml : staff.role}
                  </p>
                </div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {exp.recentExpenses
                ...
                .map((e) => (
                  <span
                    key={e.id}
                    className="text-[10px] bg-orange-100 text-orange-800 font-semibold px-2 py-0.5 rounded-full"
                  >

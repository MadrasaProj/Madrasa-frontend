    ...
    <DashboardLayout>
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <h1 className="text-xl font-bold">
                {lang === "ml" ? "ഫിനാൻഷ്യൽ സ്ഥിതി" : "Financial Status"}
              </h1>
            </div>
          </div>
          {/* Big progress bar */}
          <div className="bg-white/10 rounded-2xl p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-2xl font-black text-white">
                {d.fees.collectionPct}%
              </span>
            </div>
            <div className="h-4 bg-white/20 rounded-full overflow-hidden">
              <div
                className="h-full bg-white rounded-full transition-all duration-1000"
                style={{ width: `${d.fees.collectionPct}%` }}
              />
            </div>
            <div className="flex justify-between mt-2 text-xs text-teal-200">
              <span>₹0</span>
              <span>
                ₹{(d.fees.totalAnnualTarget / 1000).toFixed(0)}K{" "}
        ...
        <div className="flex items-center gap-3 mb-3">
          <div className="flex-1 h-10 rounded-2xl overflow-hidden flex">
            <div
              ...
              style={{
                width: `${pct(d.fees.paidStudents, d.overview.totalStudents)}%`,
              }}
            >
              {d.fees.paidStudents}
            </div>
        ...
        <div className="flex gap-6 text-xs">
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-semibold text-gray-700">
              {lang === "ml" ? "അടച്ചു" : "Paid"}: {d.fees.paidStudents}
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded bg-orange-400" />
            <span className="font-semibold text-gray-700">
              {lang === "ml" ? "ബാക്കി" : "Pending"}: {d.fees.unpaidStudents}
            </span>
          </div>
        </div>
      </motion.div>
      ...
      >
        <div className="flex items-center gap-2 mb-5">
          ...
          <p className="font-bold text-gray-800">
            {lang === "ml"
              ? "മാസ തിരിച്ച് ഫീസ് ശേഖരണം"
              : "Monthly Fee Collection"}
          </p>
        </div>
        <div className="space-y-4">
          {d.fees.monthlyTrend.map((m, i) => {
            const achieved = pct(m.collected, m.target ?? 0);
            const over = m.collected >= (m.target ?? 0);
            return (
              <motion.div
                key={m.month}
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-sm font-bold text-gray-700 w-10">
                    {lang === "ml" ? m.month_ml || m.month : m.month}
                  </span>
                  <div className="flex items-center gap-2 text-xs">
                    <span
                      className={`font-bold ${over ? "text-teal-700" : "text-amber-600"}`}
                    >
                      ₹{(m.collected / 1000).toFixed(1)}K
                    </span>
                    <span className="text-gray-400">
                      / ₹{((m.target ?? 0) / 1000).toFixed(0)}K
                    </span>
                    {over ? (
                ...
                <Bar
                  value={achieved}
                  color={over ? "bg-teal-500" : "bg-amber-400"}
                  height="h-2.5"
                />
              </motion.div>
            );

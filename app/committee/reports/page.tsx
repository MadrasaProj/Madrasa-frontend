...
function computeHealthScore(
  d: ReturnType<typeof import("@/lib/use-committee-data").useCommitteeData>,
) {
...
function getStatus(v: number, lang: string) {
  if (v >= 85)
    return {
      label: lang === "ml" ? "ഉത്തമം" : "Excellent",
  ...
  return {
    label: lang === "ml" ? "ദുർബലം" : "Needs Work",
    color: "text-red-600",
    bg: "bg-red-50",
    bar: "bg-red-400",
  ...
  const sections = [
    {
      ...
      details: [
        {
          k: lang === "ml" ? "ശേഖരണം" : "Collection",
          v: `₹${(d.fees.collectedSoFar / 1000).toFixed(0)}K / ₹${(d.fees.totalAnnualTarget / 1000).toFixed(0)}K`,
        },
        {
          k: lang === "ml" ? "ബാക്കി" : "Pending",
          v: `₹${(d.fees.pendingAmount / 1000).toFixed(0)}K (${d.fees.unpaidStudents} ${lang === "ml" ? "വിദ്യാർ." : "students"})`,
        },
      ],
    },
    {
      ...
      details: [
        {
          k: lang === "ml" ? "ഒ.ഹാജർ %" : "Avg %",
          v: `${d.attendance.overallPct}%`,
        },
        {
          k: lang === "ml" ? "ഇന്ന്" : "Today",
          v: `${d.attendance.todayPresent} / ${d.attendance.todayPresent + d.attendance.todayAbsent}`,
        },
        {
          k: lang === "ml" ? "ശ്രദ്ധ ആവശ്" : "Attention",
          v: `${d.attendance.lowAttendanceStudents} ${lang === "ml" ? "വിദ്യാർ." : "students"}`,
        },
      ],
    },
    {
      ...
      details: [
        {
          k: lang === "ml" ? "ശ.ശ. സ്കോർ" : "Avg Score",
          v: `${d.academic.lastExamAvgScore ?? 0}%`,
        },
        {
          k: lang === "ml" ? "വിജയ %" : "Pass Rate",
          v: `${d.academic.passRate}%`,
        },
        {
          k: lang === "ml" ? "ടോപ്പ് വിദ്യാർ." : "Toppers",
          v: `${(d.academic.topStudents || []).length}`,
        },
      ],
    },
    {
      ...
      details: [
        {
          k: lang === "ml" ? "ഖുർആൻ %" : "Quran %",
          v: `${d.ibadah.quranCompletionPct}%`,
        },
        {
          k: lang === "ml" ? "നമ. ട്രാക്ക്" : "Prayer Tracked",
          v: `${(d.ibadah.prayerTrackedStudents || []).length ?? 0}`,
        },
        {
          k: lang === "ml" ? "ചാമ്പ്യൻസ്" : "Champions",
          v: `${(d.ibadah.ibadahChampions || []).length}`,
        },
      ],
    },
  ];
    ...
    <DashboardLayout>
      ...
      >
        <div className="bg-linear-to-r from-slate-700 to-gray-600 rounded-3xl p-5 lg:p-6 text-white">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <h1 className="text-xl font-bold">
                {lang === "ml"
                  ? "മദ്‌റസ ആരോഗ്യ റിപ്പോർട്ട്"
                  : "Madrasa Health Report"}
              </h1>
            </div>
          </div>
          {/* Big health gauge */}
          <div className="bg-white/10 rounded-2xl p-4 flex items-center gap-4">
            <div
              className={`w-16 h-16 rounded-full bg-white flex items-center justify-center shadow-lg shrink-0`}
            >
              <span className={`text-xl font-black ${overallStatus.color}`}>
                {health}
              </span>
            </div>
            <div className="flex-1">
              <p className="text-white font-bold text-base">
                {lang === "ml"
                  ? "ഒट्टाकে ആരോഗ്യ സ്കോർ"
                  : "Overall Health Score"}
              </p>
              <p
                className={`text-sm font-semibold ${overallStatus.color.replace("text-", "text-")} bg-white/20 rounded-lg px-2 py-0.5 inline-block mt-1`}
              >
                {overallStatus.label}
              </p>
              <div className="h-2.5 bg-white/20 rounded-full overflow-hidden mt-2">
                <div
                  className={`h-full ${overallStatus.bar} rounded-full`}
                  style={{ width: `${health}%` }}
                />
              </div>
            </div>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div
                    className={`w-8 h-8 ${st.bg} rounded-xl flex items-center justify-center`}
                  >
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span
                    className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${st.bg} ${st.color}`}
                  >
                    {st.label}
                  </span>
                </div>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-y-3 text-sm">
          {[
            { k: lang === "ml" ? "പേര്" : "Name", v: d.madrasa?.name ?? "—" },
            {
              k: lang === "ml" ? "സ്ഥലം" : "Location",
              v: d.madrasa?.location ?? "—",
            },
            {
              k: lang === "ml" ? "സ്ഥാപിതം" : "Established",
              v: d.madrasa?.established ?? "—",
            },
            {
              k: lang === "ml" ? "അദ്ധ്യ. വർഷം" : "Session",
              v: d.madrasa?.session ?? "—",
            },
          ].map((row) => (
            <div key={row.k}>
      ...
      >
        ...
        <div>
          <p
            className={`text-sm font-bold ${health >= 80 ? "text-emerald-800" : "text-amber-800"}`}
          >
            {health >= 80
              ? lang === "ml"
                ? "മദ്‌റസ മൊത്തത്തിൽ നന്നായി പ്രവർത്തിക്കുന്നു"
                : "Madrasa is performing well overall"
              : lang === "ml"
                ? "ചില മേഖലകൾ ശ്രദ്ധ ആവശ്യമാണ്"
                : "Some areas need attention"}
          </p>
          <p
            className={`text-xs mt-0.5 ${health >= 80 ? "text-emerald-600" : "text-amber-600"}`}
          >
            {lang === "ml"
      ...
      >
        ...
        <div className="flex items-end gap-1.5 h-20">
          {d.fees.monthlyTrend.map((m, i) => {
            const max = Math.max(
              ...d.fees.monthlyTrend.map((x) => x.collected),
            );
            ...
            return (
              ...
              >
                <div
                  className="w-full bg-slate-500 rounded-t"
                  style={{ height: `${h * 0.55}px`, minHeight: "4px" }}
                />
                <p className="text-[9px] text-gray-400 font-medium">
                  {lang === "ml" ? m.month_ml : m.month}
                </p>
              </motion.div>
            );
          })}

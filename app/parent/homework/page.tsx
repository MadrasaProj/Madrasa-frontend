...
const STATUS_CONFIG: Record<HomeworkStatus, { label: string; color: string; icon: React.ElementType }> = {
  NOT_SUBMITTED: { label: "Pending",   color: "bg-red-100 text-red-700",       icon: AlertCircle   },
  SUBMITTED:     { label: "Submitted", color: "bg-amber-100 text-amber-700",   icon: Clock         },
  CHECKED:       { label: "Checked",   color: "bg-emerald-100 text-emerald-700", icon: CheckCircle2 },
};
...
export default function ParentHomeworkPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [student, hw] = await Promise.all([
        ...
        getStudentHomework(cid, token, effectiveId).catch((e: Error) => ({ error: e.message })),
      ]);
      if ("error" in hw) {
        setActive({ studentId: effectiveId, student: student as StudentRecord, hw: null, error: (hw as any).error });
      } else {
        setActive({ studentId: effectiveId, student: student as StudentRecord, hw, error: null });
      }
    } catch (e) {
      setActive({ studentId: effectiveId, student: null, hw: null, error: (e as Error).message });
    }
    setLoading(false);
  ...
  const filtered = active?.hw?.homework.filter((hw) =>
    filter === "all" || hw.submission.status === filter,
  ) ?? [];

  const pendingCount = active?.hw?.homework.filter((hw) => hw.submission.status === "NOT_SUBMITTED").length ?? 0;
          ...
          {active?.error ? (
            ...
            <>
              ...
              {pendingCount > 0 && (
                <div className="flex items-center gap-2 bg-red-50 border border-red-100 rounded-2xl px-4 py-3 mb-4">
                  ...
                  <p className="text-sm text-red-700">
                    <span className="font-bold">{pendingCount}</span> pending homework{pendingCount !== 1 ? "s" : ""}
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-3 gap-2 mb-5">
                {(["all", "NOT_SUBMITTED", "SUBMITTED", "CHECKED"] as const).slice(1).map((s) => {
                  const count = active.hw!.homework.filter((hw) => hw.submission.status === s).length;
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        filter === s ? `${cfg.color} border-current` : "bg-white border-gray-100",
                      )}
                    >
              ...
              <div className="flex gap-1.5 mb-4 bg-gray-100 p-1 rounded-xl w-fit">
                {(["all", "NOT_SUBMITTED", "SUBMITTED", "CHECKED"] as const).map((f) => (
                  <button
                    key={f}
                    onClick={() => setFilter(f)}
                    className={cn(
                ...
                {filtered.length === 0 ? (
                  ...
                  return (
                    <motion.div
                      ...
                      className={cn("bg-white rounded-2xl border-2 p-4", urgencyColor(hw.dueDate, sub.status))}
                    >
                      <div className="flex items-start justify-between gap-3 mb-2">
                        ...
                        <span className={cn("text-[10px] font-bold px-2 py-1 rounded-lg shrink-0 flex items-center gap-1", cfg.color)}>
                      ...
                      <div className="flex items-center justify-between">
                        <p className={cn("text-xs flex items-center gap-1", isOver && sub.status === "NOT_SUBMITTED" ? "text-red-500 font-semibold" : "text-gray-400")}>
                          <Calendar className="w-3 h-3" />
                          Due {due.toLocaleDateString("en-GB", { day: "numeric", month: "short" })}
                          {sub.status === "NOT_SUBMITTED" && (
                            <span className={cn("ml-1", isOver ? "text-red-500" : daysLeft <= 1 ? "text-amber-600" : "text-gray-400")}>
                              {isOver ? "(overdue!)" : daysLeft === 0 ? "(today)" : `(${daysLeft}d left)`}
                            </span>
                          )}
                        </p>
                        {sub.submittedAt && (

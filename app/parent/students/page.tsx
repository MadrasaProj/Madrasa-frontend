...
import { getStudentProfileV2, uploadStudentPhoto, type StudentRecord } from "@/lib/students-api";
import { getStudentAttendance, type StudentAttendanceResponse } from "@/lib/attendance-api";
import { useAuthStore } from "@/store/auth";
import { motion, AnimatePresence } from "framer-motion";
import { User, Phone, Calendar, Loader2, GraduationCap, Hash, Heart, MapPin, AlertCircle, Pencil, Upload, X, CheckCircle } from "lucide-react";
...
export default function ParentStudentProfile() {
 ...
 const { user, accessToken, activeClientId, activeStudentId, setActiveStudent } = useAuthStore();
 ...
 bloodGroup: "", emergencyContactName: "", emergencyContactPhone: "", medicalNotes: "",
 });
 ...
 getStudentAttendance(activeClientId, accessToken, studentId,
 ...
 const res = await fetch(`${V2_BASE}/${activeClientId}/students/${student.id}`, {
 method: "PATCH",
 headers: {
 ...
 const updated = await getStudentProfileV2(activeClientId, accessToken, student.id);
 setStudent(updated);
 setShowEdit(false);
 ...
 try {
 await uploadStudentPhoto(activeClientId, accessToken, student.id, avatarFile);
 setAvatarFile(null);
 ...
 const studentName = students.find((s) => s.id === studentId)?.name ?? student.name;
 ...
 const inputCls = "w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50 focus:outline-none focus:border-emerald-400 text-sm";
 ...
 return (
 <button key={id} onClick={() => switchStudent(id)}
 className={cn(
 ...
 GENDER_COLORS[student.gender ?? ""] ?? "bg-gray-100 text-gray-600",
 )}>
 ...
 <a href="/parent/profile" className="font-semibold underline">Parent Profile</a>.
 ...
 setEditError((err as Error).message ?? "Photo upload failed");
 } finally {
 setUploadingPhoto(false);
 ...
 {/* Health & Emergency */}
 <section>
 <p className="text-xs font-bold text-amber-700 uppercase tracking-wide mb-3">Health & Emergency</p>
 ...
 <input value={editForm.emergencyContactName}
 onChange={(e) => setEditForm(f => ({ ...f, emergencyContactName: e.target.value }))}
 className={inputCls} placeholder="Emergency contact person" />
 </div>
 <div>
 <label className={labelCls}>Emergency Contact Phone</label>
 <input value={editForm.emergencyContactPhone}
 ...
 rows={2} className={inputCls + " resize-none"} placeholder="Allergies, conditions, notes" />
 </div>
 </div>
 </section>
 </div>

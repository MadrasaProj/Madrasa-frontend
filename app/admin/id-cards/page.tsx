...
const THEMES: { id: Theme; label: string; desc: string }[] = [
  {
    id: "classic",
    label: "Classic Green",
    desc: "Traditional design, emerald header",
  },
  {
    id: "modern",
    label: "Modern Dark",
    desc: "Navy gradient, bold typography",
  },
  {
    id: "minimal",
    label: "Minimal White",
    desc: "Clean white with coloured side stripe",
  },
  {
    id: "islamic",
    label: "Islamic Pattern",
    desc: "Gold accents with decorative border",
  },
];
...
function getColor(cls: string) {
  return classColors[cls] ?? defaultColor;
}
...
// Card settings type and default
type IdCardSettings = {
  madrasaName: string;
...
const DEFAULT_CARD_SETTINGS: IdCardSettings = {
  madrasaName: "Madrasa",
  madrasaAddress: "",
  madrasaPhone: "",
  logoText: "M",
...
function BarcodeStrip({ light = false }: { light?: boolean }) {
  const bars = [
    3, 1, 2, 1, 3, 2, 1, 2, 1, 3, 1, 2, 1, 1, 3, 2, 1, 2, 3, 1, 2, 1, 1, 3, 1,
    2,
  ];
  return (
    ...
    >
      <defs>
        ...
        >
          ...
          />
          <path
            d="M0 0 L20 20 M40 0 L20 20 M0 40 L20 20 M40 40 L20 20"
            stroke="currentColor"
            strokeWidth="0.4"
          />
        </pattern>
    ...
    <div className="bg-white rounded-2xl shadow-md overflow-hidden border border-gray-200 select-none w-full">
      <div className={cn("px-4 py-3 flex items-center gap-3", col.bg)}>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-white font-bold text-sm leading-tight truncate">
            {idCardSettings.madrasaName}
          </p>
          <p className="text-white/70 text-[10px] truncate">
            {idCardSettings.academicYear}
          </p>
        </div>
        <span className="text-[9px] text-white/90 font-bold border border-white/40 rounded px-1.5 py-0.5 shrink-0">
          ID
        </span>
      </div>
      <div
        className={cn("px-4 flex gap-3 items-start", mini ? "py-3" : "py-4")}
      >
        <div
          className={cn(
            "shrink-0 rounded-xl overflow-hidden flex flex-col items-center justify-center text-white font-bold shadow-sm relative",
            col.bg,
            sz,
          )}
        >
          {student.photo ? (
            ...
            <>
              ...
              <span className="text-[7px] text-white/60 mt-0.5">PHOTO</span>
            </>
          )}
        </div>
        <div className="flex-1 min-w-0 space-y-1">
          ...
          <IRow
            icon={BookMarked}
            text={`${student.class} · Div ${student.division}`}
            mini={mini}
          />
          <IRow
            icon={Calendar}
      ...
      <div className="px-4 pb-3">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <div
            className={cn(
              "px-2 py-0.5 rounded-lg text-[10px] font-bold text-white shrink-0 ml-3",
              col.bg,
            )}
          >
            {student.class}
        ...
        <p className="text-[8px] text-gray-400">
          {idCardSettings.madrasaAddress} · {idCardSettings.madrasaPhone}
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-slate-900 rounded-2xl shadow-xl overflow-hidden border border-slate-700 select-none w-full">
      <div
        ...
        style={{ background: `linear-gradient(to right, ${col.hex}, #6366f1)` }}
      />
      <div className="px-4 pt-3 pb-2 flex items-center gap-2 border-b border-slate-700/60">
        <div
          className="w-7 h-7 rounded-lg flex items-center justify-center text-white font-extrabold text-[10px] shrink-0"
        ...
        <span
          ...
          style={{ color: col.hex, borderColor: col.hex }}
        >
          STUDENT
        </span>
      </div>
      ...
      >
        <div
          className={cn(
            "shrink-0 rounded-2xl overflow-hidden border-2 flex flex-col items-center justify-center text-white font-bold",
            sz,
          )}
          style={{
            borderColor: col.hex,
            background: `linear-gradient(145deg, ${col.hex}33, ${col.hex}11)`,
          }}
        >
          {student.photo ? (
            <img
            ...
            <>
              <span className={mini ? "text-xl" : "text-2xl"}>
        ...
        <div className="flex-1 min-w-0 space-y-1">
          ...
          <p className="text-[10px] font-semibold" style={{ color: col.hex }}>
            {student.class} · Division {student.division}
          </p>
          <div className="space-y-0.5 mt-1">
            ...
            {!mini && (
              <IRowDark icon={User} text={student.motherName} mini={false} />
            )}
            <IRowDark icon={Phone} text={student.phone} mini={mini} />
            {!mini && (
              <IRowDark icon={MapPin} text={student.address} mini={false} />
            )}
          </div>
        </div>
      </div>
      <div className="px-4 pb-3 flex items-center justify-between">
        <BarcodeStrip light />
        <p className="text-[8px] text-slate-500 ml-3 truncate">
          {idCardSettings.madrasaPhone}
        </p>
      </div>
    </div>
  );
    ...
    <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-200 select-none w-full flex">
      ...
      <div className="flex-1 min-w-0">
        <div className="px-4 py-2 flex items-center justify-between border-b border-gray-100">
          <div>
            <p className="font-extrabold text-gray-900 text-xs leading-tight">
              {idCardSettings.madrasaName}
            </p>
          ...
          <div
            className={cn(
              "w-7 h-7 rounded-lg flex items-center justify-center text-white text-[10px] font-extrabold shrink-0",
              col.bg,
            )}
          >
            {idCardSettings.logoText}
        ...
        >
          <div
            className={cn(
              "shrink-0 rounded-xl overflow-hidden border-2 flex flex-col items-center justify-center text-white font-bold",
              col.bg,
              mini ? "w-12 h-14" : "w-18 h-22",
            )}
          ...
          >
            {student.photo ? (
              ...
              <>
                <User
                  className={cn("opacity-50", mini ? "w-5 h-5" : "w-7 h-7")}
                />
                <span className="text-[7px] text-white/60 mt-0.5">PHOTO</span>
              </>
            )}
          ...
          <div className="flex-1 min-w-0 space-y-1">
            ...
            <span
              className={cn(
                "inline-block text-[9px] font-semibold px-1.5 py-0.5 rounded-full text-white",
                col.bg,
              )}
            >
              {student.class}
            ...
            <div className="space-y-0.5 mt-0.5">
              ...
              {!mini && (
                <IRow icon={User} text={student.motherName} mini={false} />
              )}
              <IRow icon={Phone} text={student.phone} mini={mini} />
              {!mini && (
                <IRow icon={MapPin} text={student.address} mini={false} />
              )}
            </div>
          </div>
        </div>
    ...
    <div className="bg-amber-700 rounded-2xl shadow-lg overflow-hidden border-2 border-amber-400/40 select-none w-full relative text-white">
      ...
      <div className="px-4 pt-3 pb-2 flex items-center gap-2 border-b border-amber-500/40 relative z-10">
        <div className="w-8 h-8 rounded-full border-2 border-yellow-300 flex items-center justify-center bg-amber-800 shrink-0">
          <p className="text-yellow-300 font-extrabold text-[10px]">
            {idCardSettings.logoText}
          </p>
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-yellow-100 font-extrabold text-xs leading-tight truncate">
            {idCardSettings.madrasaName}
          </p>
          <p className="text-amber-300 text-[9px]">
            {idCardSettings.academicYear}
          </p>
        </div>
        <span className="text-[9px] text-yellow-300 font-bold border border-yellow-400/50 rounded px-1.5 py-0.5 shrink-0">
          ID
        </span>
      </div>
      <div
        className={cn(
          "px-4 flex gap-3 items-start relative z-10",
          mini ? "py-3" : "py-4",
        )}
      >
        <div
          className={cn(
            "shrink-0 rounded-xl overflow-hidden border-2 border-yellow-300/60 bg-amber-800 flex flex-col items-center justify-center text-white font-bold relative",
        ...
        >
          {student.photo ? (
            ...
            <>
              ...
              <span className="text-[7px] text-yellow-300/60 mt-0.5">
                PHOTO
              </span>
            </>
          )}
        ...
        <div className="flex-1 min-w-0 space-y-1">
          <p
            className={cn(
              "font-extrabold text-yellow-100 leading-tight",
              mini ? "text-xs" : "text-sm",
            )}
          >
            {student.name}
          ...
          <p className="text-yellow-300 text-[10px] font-semibold">
            {student.class} · Div {student.division}
          </p>
          <div className="space-y-0.5">
            ...
            {!mini && (
              <IRowLight icon={User} text={student.motherName} mini={false} />
            )}
            <IRowLight icon={Phone} text={student.phone} mini={mini} />
            {!mini && (
              <IRowLight icon={MapPin} text={student.address} mini={false} />
            )}
          </div>
        </div>
      </div>
      <div className="px-4 pb-3 border-t border-amber-500/40 pt-2 relative z-10 flex items-center justify-between">
        <BarcodeStrip light />
        <p className="text-[8px] text-amber-300 ml-3 truncate">
          {idCardSettings.madrasaPhone}
        </p>
      </div>
      <div
    ...
    <div className="flex items-center gap-1.5">
      <Icon
        className={cn(
          ...
          mini ? "w-2.5 h-2.5" : "w-3.5 h-3.5",
        )}
      />
      <span
        className={cn(
          "text-gray-700 leading-tight truncate",
          mini ? "text-[9px]" : "text-xs",
        )}
      >
        {text}
      </span>
    ...
    <div className="flex items-center gap-1.5">
      <Icon
        className={cn(
          ...
          mini ? "w-2.5 h-2.5" : "w-3 h-3",
        )}
      />
      <span
        className={cn(
          "text-slate-300 leading-tight truncate",
...
function IRowLight({
  ...
  return (
    <div className="flex items-center gap-1.5">
      <Icon
        className={cn(
          "shrink-0 text-amber-300/70",
          mini ? "w-2.5 h-2.5" : "w-3 h-3",
        )}
      />
      <span
        className={cn(
          "text-amber-100/80 leading-tight truncate",
          mini ? "text-[9px]" : "text-[10px]",
        )}
      >
        {text}
    ...
    <DashboardLayout>
        ...
        action={
          <div className="flex gap-2 flex-wrap">
            ...
            >
              ...
              {bulkMode
                ? `${selected.size} ${tr("common", "selected", lang)}`
                : tr("common", "bulkSelect", lang)}
            </button>
      ...
      <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 mb-4 flex items-center gap-4">
        ...
        <div className="text-right shrink-0">
          <span className="text-xs font-bold bg-emerald-600 text-white px-3 py-1.5 rounded-lg">
            {idCardSettings.academicYear}
          </span>
          <p className="text-[10px] text-emerald-500 mt-1">
            {students.length} {tr("adminPages", "totalStudentsLabel", lang)}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {classStats.map(({ cls, count, color }) => (
          ...
          >
            <div
              className={cn(
                "w-6 h-6 rounded-lg flex items-center justify-center mb-1.5 text-white text-[10px] font-bold",
                color.bg,
              )}
            >
              {count}
            ...
            <p
              className={cn(
                "text-xs font-bold",
                activeClass === cls ? color.text : "text-gray-700",
              )}
            >
              {cls}
            </p>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 mb-5">
        {ALL_CLASSES.map((cls) => (
          <button
            ...
            className={cn(
              "px-4 py-2 rounded-xl text-sm font-semibold whitespace-nowrap shrink-0 transition-all border",
              activeClass === cls
            ...
            >
              {bulkMode && (
                <button
                  ...
                  className={cn(
                    "absolute top-2.5 left-2.5 z-10 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all shadow-sm",
                    isSel
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                  <div>
                    <p className="font-bold text-gray-900">
                      {tr("adminPages", "idCardPreview", lang)}
                    </p>
                    <p className="text-xs text-gray-400">
                      {previewStudent.name} · {previewStudent.admissionNumber}
                    </p>
                  </div>
                  <button
                    onClick={() => setPreview(null)}
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <div className="flex items-center gap-2">
                  ...
                  <p className="font-bold text-gray-900">
                    {tr("adminPages", "chooseTheme", lang)}
                  </p>
                </div>
              ...
              <div className="p-5 space-y-4">
                <p className="text-sm text-gray-500">
                  Select a design template. Changes apply to all cards
                  instantly.
                </p>
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <div className="flex items-center gap-2">
                  ...
                  <p className="font-bold text-gray-900">
                    {tr("adminPages", "cardSettings", lang)}
                  </p>
                </div>
              ...
              <div className="p-5 space-y-5">
                <SettingField
                  label="Madrasa Name"
                  defaultValue={idCardSettings.madrasaName}
                />
                <SettingField
                  label="Address"
                  defaultValue={idCardSettings.madrasaAddress}
                />
                <SettingField
                  label="Phone"
                  defaultValue={idCardSettings.madrasaPhone}
                />
                <SettingField
                  label="Academic Year"
                  defaultValue={idCardSettings.academicYear}
                />
                <SettingField
                  label="Logo Text"
                  defaultValue={idCardSettings.logoText}
                />
                <div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 space-y-3">
                    {[
                      ["Show photo if available", true],
                      ["Fallback: initials avatar", true],
                      ["Gender-aware avatar color", false],
                    ].map(([label, on]) => (
                      <div
                        key={label as string}
                  ...
                  <div className="space-y-2">
                    {classStats.map(({ cls, color }) => (
                      ...
                      >
                        ...
                        <p className={cn("text-[10px] font-bold", color.text)}>
                          Active
                        </p>
                      </div>
                    ))}

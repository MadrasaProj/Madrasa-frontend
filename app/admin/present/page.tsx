...
export default function AdminPresentPage() {
  ...
  useEffect(() => { load(); }, [activeClientId, accessToken]); // eslint-disable-line react-hooks/exhaustive-deps

  const present = records.filter((r) => r.status === "PRESENT");

  // Group by class name
  const byClass = present.reduce<Record<string, ClassAttendanceRecord[]>>((acc, r) => {
  ...
  return (
    <DashboardLayout>
      ...
      {!loading && !error && (
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <span className="text-2xl font-bold text-emerald-700">{present.length}</span>
            <span className="text-sm text-gray-500">{t("common", "present", lang)} today</span>
          </div>
      ...
      {loading ? (
        ...
        <div className="px-4 py-4 text-sm text-red-600 bg-red-50 rounded-2xl">{error}</div>
      ) : present.length === 0 ? (
        <div className="py-12 text-center text-sm text-gray-400">No attendance marked yet for today</div>
      ) : (
        <div className="space-y-4">
          {Object.entries(byClass).sort(([a], [b]) => a.localeCompare(b)).map(([className, students]) => (

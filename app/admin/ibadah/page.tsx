...
export default function AdminIbadahPage() {
  ...
  const stats = useMemo(() => ({
    total: rows.length,
    presentAll: rows.filter((r) => activePrayers.every((p) => r[p.key])).length,
    quranDone: rows.filter((r) => r.quranPages > 0).length,
  }), [rows, activePrayers]);
  ...
  return (
    <DashboardLayout>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl px-4 py-2.5 mb-4 flex items-center gap-2 text-xs text-blue-700">
        ...
        Parents submit ibadah for their children. Viewing records only.
      </div>
      ...
      {loadingClasses ? (
        ...
        <div className="text-center py-16 text-gray-400 text-sm">No classes found</div>
      ) : (
        <>
              ...
              {viewMode === "daily" && (
                rows.length === 0 ? (
                  <div className="text-center py-12 text-gray-400 text-sm">
                    ...
                    No ibadah submitted for this date.
                  </div>
                ) : (
                  <>
                    {sections.length > 1 && (
                    ...
                    {activeSection === "quran" && config?.enableQuranPages !== false && (
                      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden mb-4">
                        <div className="px-4 py-2.5 bg-gray-50 flex items-center gap-2">
                          ...
                          <p className="text-xs font-bold text-gray-500 uppercase">Quran Pages</p>
                        </div>
                ...
                weeklyStudents.length === 0 ? (
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                                ...
                                {weekDates.map((d) => {
                                  ...
                                  return (
                                    <td key={d} className="px-2 py-3 text-center align-top">
                                      {!log ? (
                                        <div className="w-10 h-7 mx-auto rounded-lg bg-gray-50 text-[10px] text-gray-300 flex items-center justify-center">—</div>
                                      ) : (
                    ...
                    <div className="px-4 py-3 bg-gray-50 border-t border-gray-100 flex gap-4 text-xs text-gray-500">
                      ...
                      <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-gray-50 border inline-block" /> Not recorded</div>
                    </div>
                  </div>
                )
              )}

...
import {
  ...
  Plus, Trash2, GripVertical, ToggleLeft, Hash,
} from "lucide-react";
...
const PRAYER_FIELDS: { key: keyof IbadahConfig; label: string; desc: string }[] = [
  ...
  { key: "enableQuranPages", label: "Quran Pages", desc: "Daily Quran reading tracker" },
];
...
function validateKey(key: string): string | null {
  ...
  if (!/^[a-z][a-z0-9_]*$/.test(key)) return "Lowercase letters, digits, underscores only, start with letter";
  return null;
}
...
export default function IbadahConfigPage() {
  ...
  const handleAddItem = () => {
    ...
    if (keyErr) { setNewItemError(keyErr); return; }
    if (!newItem.label.trim()) { setNewItemError("Label is required"); return; }
  ...
  const handleSaveEdit = () => {
    ...
    if (keyErr) { setEditItemError(keyErr); return; }
    if (!editItem.label.trim()) { setEditItemError("Label is required"); return; }
    if (customItems.some((i, idx) => i.key === editItem.key && idx !== editingIdx)) {
    ...
    setCustomItems((prev) => prev.map((ci, i) => (i === editingIdx ? item : ci)));
    setEditingIdx(null);
    setEditItemError(null);
  };
  ...
  if (!isSuperAdmin) {
    return (
      <DashboardLayout>
        <PageHeader title="Ibadah Configuration" subtitle="Global settings" icon={Moon} />
        <div className="bg-red-50 text-red-600 text-sm px-4 py-3 rounded-2xl flex items-center gap-2 mt-4">
          <AlertCircle className="w-4 h-4 shrink-0" /> Super Admin access required.
        </div>
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout>
      ...
      {loading ? (
        ...
        <div className="space-y-5 max-w-2xl pb-28">
          ...
          >
            <p className="text-xs font-bold text-emerald-600 uppercase tracking-wide mb-4">Standard Ibadah Items</p>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <p className="text-xs font-bold text-emerald-600 uppercase tracking-wide">Custom Items</p>
              {!addingItem && (
                <button
                  onClick={() => { setAddingItem(true); setNewItem({ ...EMPTY_ITEM }); setNewItemError(null); }}
            ...
            {customItems.length === 0 && !addingItem && (
              <p className="text-sm text-gray-400 text-center py-4">No custom items. Add one to track additional ibadah.</p>
            )}
                  ...
                  >
                    {editingIdx === idx ? (
                      <CustomItemForm
                        ...
                        onCancel={() => { setEditingIdx(null); setEditItemError(null); }}
                        mode="edit"
                      />
                    ) : (
                      <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl group">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-mono text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded shrink-0">{item.key}</span>
                          </div>
                          {item.type === "number" && (
                ...
                {addingItem && (
                  ...
                  >
                    <CustomItemForm
                      item={newItem}
                      onChange={(item) => {
                        if (!newItem.key || newItem.key === generateKey(newItem.label)) {
                      ...
                      onCancel={() => { setAddingItem(false); setNewItemError(null); }}
                      mode="add"
                    />
                  </motion.div>
                )}
          ...
          {config && (
            <div className="bg-amber-50 border border-amber-100 rounded-2xl px-4 py-3 text-xs text-amber-700">
              Changes apply globally to all madrasas. Teachers will see updated items on next page load.
            </div>
          )}
...
function CustomItemForm({ item, onChange, error, onSave, onCancel, mode }: CustomItemFormProps) {
  return (
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input
            type="text"
            value={item.key}
            onChange={(e) => onChange({ ...item, key: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, "") })}
            placeholder="e.g. tahajjud"
          ...
          {(["boolean", "number"] as const).map((t) => (
            <label
              ...
              className={cn(
                ...
                item.type === t ? "border-emerald-500 bg-emerald-50 text-emerald-700" : "border-gray-200 text-gray-600",
              )}
            >
              <input
                type="radio"
                ...
                className="sr-only"
              />
              {t === "boolean" ? <ToggleLeft className="w-4 h-4" /> : <Hash className="w-4 h-4" />}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              value={item.min ?? 0}
              min={0}
              onChange={(e) => onChange({ ...item, min: Math.max(0, Number(e.target.value)) })}
              className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white"
            />
          </div>
          <div>
            ...
            <input
              ...
              min={1}
              max={10000}
              onChange={(e) => onChange({ ...item, max: Math.min(10000, Math.max(1, Number(e.target.value))) })}
              className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white"
            />
          </div>
        </div>

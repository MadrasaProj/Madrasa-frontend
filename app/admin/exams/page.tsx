...
export default function AdminExamsPage() {
  ...
  const [deleteExamTarget, setDeleteExamTarget] = useState<ExamRecord | null>(
    null,
  );
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (drawerMode === "create") {
        ...
        setExams((prev) =>
          prev.map((ex) => (ex.id === expandedId ? { ...ex, ...res } : ex)),
        );
      }
      setDrawerOpen(false);
      setForm(EMPTY_FORM);
        ...
        {pageError && (
          <div className="bg-rose-50 border border-rose-100 text-rose-600 text-sm px-4 py-3 rounded-2xl flex items-center gap-2">
            ...
            <button
              ...
              className="ml-auto p-1 text-rose-400 hover:text-rose-600"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredExams.map((exam) => {
              ...
              return (
                <div
                  key={exam.id}
                  className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md/50 transition-shadow"
                >
                  {/* Card Header Section */}
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500 mt-1">
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            {exam.endDate
                              ? fmt(
                                  new Date(
                                    new Date(exam.endDate).getTime() + 86400000,
                                  ).toISOString(),
                                )
                              : "—"}{" "}
                            ...
                            {classes.map((cls) => (
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2">
                                  <button
                                    onClick={() =>
                                      goToClassReport(exam.id, cls.id)
                                    }
                            ...
                            <div className="bg-white rounded-3xl border border-emerald-100 p-5 space-y-4 shadow-sm">
                              <div className="flex items-center justify-between border-b pb-2">
                                <p className="text-xs font-bold text-emerald-700 uppercase tracking-wide">
                                  ...
                                  {
                                    classes.find(
                                      (c) => c.id === markEntry.classId,
                                    )?.name
                                  }
                                </p>
                                <button
                                  onClick={() => setMarkEntryOpen(null)}
              ...
              >
                ...
                <div className="space-y-4">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                    <GraduationCap className="w-4 h-4 text-emerald-600" /> Basic
                    Information
                  </p>
                  <div>

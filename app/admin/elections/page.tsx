...
function StatusPill({ status, lang }: { status: string; lang: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${m.color} ${m.bg}`}
    >
...
function CategoryChip({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${m.color} ${m.bg}`}
    >
      ...
      >
        <div className="bg-linear-to-r from-emerald-600 to-teal-600 rounded-t-3xl p-6 text-white">
            ...
            <div>
              <div className="flex flex-wrap gap-2 mb-2">
                ...
                <span className="text-xs bg-white/20 px-2.5 py-1 rounded-full font-semibold">
                  ...
                  {lang === "ml"
                    ? CATEGORY_META[election.category].label_ml
                    : CATEGORY_META[election.category].label}
                </span>
              </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <span className="text-xs bg-white/15 px-2.5 py-1 rounded-full">
              {lang === "ml" ? "ആകെ വോട്ടുകൾ" : "Total Votes"}: {totalVotes}/
        ...
        {winner && (
          <div className="mx-5 mt-5 bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-4">
            ...
            <div className="flex-1">
              <p className="text-amber-600 text-xs font-bold uppercase tracking-wide">
                🏆 {lang === "ml" ? "ജേതാവ്" : "Winner"}
              </p>
              <p className="font-bold text-gray-900">{winner.name}</p>
              <p className="text-xs text-gray-500">
                {winner.class} · {winner.voteCount}{" "}
                {lang === "ml" ? "വോട്ടുകൾ" : "votes"}
              </p>
            </div>
        ...
        <div className="px-5 pt-4">
          <p className="text-sm text-gray-600 leading-relaxed">
            {lang === "ml" ? election.description_ml : election.description}
          </p>
        </div>
            ...
            {sorted.map((c, i) => (
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 bg-white rounded-xl border border-gray-200 flex items-center justify-center text-lg shrink-0">
                    {c.symbol}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-gray-700">
                    {c.voteCount} {lang === "ml" ? "വോട്ട്" : "votes"}
                  </span>
                </div>
                <ProgressBar
                  value={c.voteCount}
        ...
        <div className="mx-5 mb-5 bg-gray-50 rounded-2xl p-4 border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            {election.sessionOpenedAt && (
              <div>
                <p className="text-xs text-gray-500">
                  {lang === "ml" ? "സെഷൻ തുറന്നു" : "Session Opened"}
                </p>
                <p className="font-semibold text-gray-800">
                  {new Date(election.sessionOpenedAt).toLocaleTimeString()}
                </p>
              </div>
            )}
            {election.sessionClosedAt && (
              <div>
                <p className="text-xs text-gray-500">
                  {lang === "ml" ? "സെഷൻ അടഞ്ഞു" : "Session Closed"}
                </p>
                <p className="font-semibold text-gray-800">
                  {new Date(election.sessionClosedAt).toLocaleTimeString()}
                </p>
              </div>
            )}
          </div>
...
function ElectionCard({
  ...
  const turnout =
    eligibleVoters === 0 ? 0 : Math.round((totalVotes / eligibleVoters) * 100);
  const topCandidate = [...election.candidates].sort(
    (a, b) => b.voteCount - a.voteCount,
  )[0];
  ...
  return (
    ...
    >
      <div
        className={`h-1 ${isType1 ? "bg-linear-to-r from-emerald-500 to-teal-500" : "bg-linear-to-r from-teal-500 to-blue-500"}`}
      />
      <div className="p-4 lg:p-5">
        <div className="flex items-start gap-3 mb-3">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 text-xl ${CATEGORY_META[election.category].bg}`}
          >
            {CATEGORY_META[election.category].emoji}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap gap-1.5 mb-1">
              ...
              <span
                className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isType1 ? "bg-emerald-100 text-emerald-700" : "bg-teal-100 text-teal-700"}`}
              >
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          <div className="bg-gray-50 rounded-xl p-2 text-center border border-gray-100">
            <p className="text-xs lg:text-sm font-bold text-gray-800">
              {election.candidates.length}
            </p>
            <p className="text-[10px] text-gray-500">
              {lang === "ml" ? "സ്ഥാനാർ." : "Candidates"}
            </p>
          </div>
          <div className="bg-emerald-50 rounded-xl p-2 text-center border border-emerald-100">
            <p className="text-xs lg:text-sm font-bold text-emerald-700">
              {totalVotes}
            </p>
            <p className="text-[10px] text-emerald-600">
              {lang === "ml" ? "വോട്ടുകൾ" : "Votes Cast"}
            </p>
          </div>
          <div className="bg-teal-50 rounded-xl p-2 text-center border border-teal-100">
            <p className="text-xs lg:text-sm font-bold text-teal-700">
              {turnout}%
            </p>
            <p className="text-[10px] text-teal-600">
              {lang === "ml" ? "ടേർനൗട്ട്" : "Turnout"}
            </p>
          </div>
        </div>
        ...
        {totalVotes > 0 && (
          <div className="bg-amber-50 border border-amber-100 rounded-2xl p-3 mb-3 flex items-center gap-3">
            ...
            <p className="text-sm font-bold text-amber-700 shrink-0">
              {topCandidate?.voteCount} {lang === "ml" ? "വോ." : "votes"}
            </p>
          </div>
        )}
                      ...
                      <div className="flex items-center gap-1.5 w-28">
                        <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${totalVotes === 0 ? 0 : (c.voteCount / totalVotes) * 100}%`,
                            }}
                          />
                        </div>
...
function CreateElectionModal({
  ...
  const setW = (patch: Partial<CreateElectionWizard>) =>
    setWizard((w) => ({ ...w, ...patch }));
  ...
  const addCandidate = () => {
    ...
    setW({
      candidates: [
        ...wizard.candidates,
        { name: "", class: "", bio: "", symbol: next },
      ],
    });
  };

  const removeCandidate = (i: number) =>
    setW({ candidates: wizard.candidates.filter((_, idx) => idx !== i) });
  ...
  const canAdvance = () => {
    ...
    if (wizard.step === 3)
      return (
        ...
        wizard.candidates.every((c) => c.name.trim() !== "")
      );
    return true;
  };
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white rounded-3xl w-full max-h-[92vh] flex flex-col shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
          ...
          <div className="flex items-center gap-1">
            {steps.map((s, i) => (
              <div key={s.num} className="flex items-center gap-1 flex-1">
                ...
                <span
                  className={`text-[10px] font-semibold hidden sm:block ${wizard.step === s.num ? "text-white" : "text-white/60"}`}
                >
                  {s.label}
                </span>
                {i < steps.length - 1 && (
                  <div
                    className={`flex-1 h-0.5 rounded-full mx-1 ${wizard.step > s.num ? "bg-white/70" : "bg-white/25"}`}
                  />
                )}
              </div>
            ))}
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {wizard.step === 1 && (
            <div>
              <p className="text-sm font-bold text-gray-800 mb-4">
                {lang === "ml"
                  ? "ഏത് പദവിക്ക് വേണ്ടിയുള്ള തിരഞ്ഞെടുപ്പ് ആണ്?"
                  : "What position is this election for?"}
              </p>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="min-w-0">
                              <p
                                className={`text-xs font-bold leading-tight ${sel ? "text-emerald-800" : "text-gray-800"}`}
                              >
                                {lang === "ml" ? m.label_ml : m.label}
                              </p>
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 rounded-2xl p-3 flex items-center gap-2 border border-gray-100">
                <span className="text-xl">
                  {wizard.category
                    ? CATEGORY_META[wizard.category].emoji
                    : "🗳️"}
                </span>
                <div>
                  <p className="text-xs text-gray-500">
                  ...
                  <p className="text-sm font-bold text-gray-800">
                    {wizard.category
                      ? lang === "ml"
                        ? CATEGORY_META[wizard.category].label_ml
                        : CATEGORY_META[wizard.category].label
                      : ""}
                  </p>
                </div>
                <div className="ml-auto">
                  <span
                    className={`text-[10px] font-bold px-2 py-1 rounded-full ${wizard.type === "parent_vote" ? "bg-emerald-100 text-emerald-700" : "bg-teal-100 text-teal-700"}`}
                  >
                    {wizard.type === "parent_vote"
                      ? "Parent Vote"
                      : "Class Vote"}
                  </span>
                </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setW({ title_ml: e.target.value })}
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setW({ startDate: e.target.value })}
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    min={1}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-400"
                    value={wizard.eligibleVoters}
                    onChange={(e) =>
                      setW({ eligibleVoters: parseInt(e.target.value) || 1 })
                    }
                  />
                </div>
              </div>
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <select
                          ...
                          value={cand.symbol}
                          onChange={(e) =>
                            updateCandidate(i, { symbol: e.target.value })
                          }
                        >
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-2">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          value={cand.class}
                          onChange={(e) =>
                            updateCandidate(i, { class: e.target.value })
                          }
                        >
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        value={cand.bio}
                        onChange={(e) =>
                          updateCandidate(i, { bio: e.target.value })
                        }
                      />
                    </div>
                  </div>
            ...
            <div className="space-y-4">
              <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4">
                <p className="text-xs font-bold text-emerald-700 uppercase tracking-wide mb-3">
                  {lang === "ml" ? "അവലോകനം" : "Review & Confirm"}
                </p>
                <div className="space-y-2 text-sm">
                  {[
                    {
                      k: lang === "ml" ? "കാറ്റഗറി" : "Category",
                      v: wizard.category
                        ? `${CATEGORY_META[wizard.category].emoji} ${CATEGORY_META[wizard.category].label}`
                        : "—",
                    },
                    {
                      k: lang === "ml" ? "തരം" : "Vote Type",
                      v:
                        wizard.type === "parent_vote"
                          ? "Parent Vote"
                          : "Class Vote",
                    ...
                    {
                      k: lang === "ml" ? "തലക്കെട്ട്" : "Title",
                      v: wizard.title,
                    },
                    {
                      k: lang === "ml" ? "പദവി" : "Position",
                      v: wizard.position,
                    },
                    {
                      k: lang === "ml" ? "ക്ലാസ്" : "Scope",
                      v: wizard.forClass,
                    },
                    {
                      k: lang === "ml" ? "തീയതി" : "Dates",
                      v: `${wizard.startDate} → ${wizard.endDate}`,
                    },
                    {
                      k: lang === "ml" ? "വോട്ടർമാർ" : "Voters",
                      v: String(wizard.eligibleVoters),
                    },
                    {
                      k: lang === "ml" ? "മദ്‌റസ" : "Madrasa",
                      v: wizard.madrasaName,
                    },
                  ].map((row) => (
              ...
              <div>
                <p className="text-xs font-bold text-gray-600 mb-2 uppercase tracking-wide">
                  {lang === "ml" ? "സ്ഥാനാർത്ഥികൾ" : "Candidates"} (
                  {wizard.candidates.length})
                </p>
              ...
              <AnimatePresence>
                {toast && (
                  ...
                  >
                    ✅{" "}
                    {lang === "ml"
                      ? "തിരഞ്ഞെടുപ്പ് വിജയകരമായി സൃഷ്‌ടിച്ചു!"
...
export default function AdminElectionsPage() {
  ...
  const [typeFilter, setTypeFilter] = useState<
    "all" | "parent_vote" | "class_vote"
  >("all");
  const [categoryFilter, setCategoryFilter] = useState<
    "all" | ElectionCategory
  >("all");
  ...
  const stats = {
    ...
    sksbv: electionsList.filter(
      (e) => CATEGORY_META[e.category].group === "SKSBV",
    ).length,
  };

  const typeTabs: {
    key: "all" | "parent_vote" | "class_vote";
    label: string;
    label_ml: string;
    icon: typeof Vote;
  }[] = [
    {
      key: "all",
      label: "All Elections",
      label_ml: "എല്ലാ തിരഞ്ഞെടുപ്പ്",
      icon: Vote,
    },
    {
      key: "parent_vote",
      label: "Parent Votes",
      label_ml: "രക്ഷിതാവ് വോട്ട്",
      icon: Users,
    },
    {
      key: "class_vote",
      label: "Class Votes",
      label_ml: "ക്ലാസ് വോട്ട്",
      icon: School,
    },
  ];
    ...
    <DashboardLayout>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                {lang === "ml"
                  ? "SKSBV · മദ്‌റസ ലീഡർ · ക്ലാസ് — എല്ലാ തിരഞ്ഞെടുപ്പ്"
                  : "SKSBV · Madrasa Leader · Class Votes"}
              </p>
            </div>
          </div>
          <motion.button
      ...
      >
        {[
          ...
          {
            label: lang === "ml" ? "Parent" : "Parent",
            value: stats.parentVote,
            icon: Users,
            color: "text-teal-700",
            bg: "bg-teal-50",
          },
          {
            label: lang === "ml" ? "Class" : "Class",
            value: stats.classVote,
            icon: School,
            color: "text-blue-700",
            bg: "bg-blue-50",
          },
          ...
          >
            <div
              className={`w-7 h-7 rounded-xl ${s.bg} flex items-center justify-center mb-1.5`}
            >
        ...
        >
          ...
          <div>
            <p className="text-sm font-bold text-emerald-800">
              {lang === "ml"
                ? `${stats.active} തിരഞ്ഞെടുപ്പ് ഇപ്പോൾ നടക്കുന്നു`
                : `${stats.active} election(s) currently active`}
            </p>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 font-medium">
            {lang === "ml"
              ? "തിരഞ്ഞെടുപ്പൊന്നും കണ്ടെത്തിയില്ല"
              : "No elections found"}
          </p>
        </div>
      ) : (

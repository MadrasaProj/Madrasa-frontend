...
export default function AdminAbsentPage() {
  ...
  const byClass = nonPresent.reduce<Record<string, ClassAttendanceRecord[]>>((acc, r) => {
  ...
  return (
    <DashboardLayout>
      ...
      {loading ? (
        ...
        <div className="py-12 text-center text-sm text-gray-400">🎉 All students present today</div>
      ) : (
                ...
                {students.map((r) => {
                  ...
                  return (
                    <div key={r.id} className="flex items-center gap-3 px-4 py-2.5">
                      ...
                      <span className={cn("text-xs font-bold px-2 py-1 rounded-lg shrink-0", meta.bg, meta.color)}>
                        {meta.label}
                      </span>
                    </div>
                  );
